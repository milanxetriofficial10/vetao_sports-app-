...
// Ensure badminton_items has views & is_top
...
$conn->query("CREATE TABLE IF NOT EXISTS badminton_likes (
    id INT AUTO_INCREMENT PRIMARY KEY,
    product_id INT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
)");
$conn->query("CREATE TABLE IF NOT EXISTS badminton_comments (
    id INT AUTO_INCREMENT PRIMARY KEY,
    product_id INT NOT NULL,
...
if(!isset($_SESSION['viewed_badminton'])) $_SESSION['viewed_badminton'] = [];
if(!in_array($id, $_SESSION['viewed_badminton'])){
    $_SESSION['viewed_badminton'][] = $id;
    $conn->query("UPDATE badminton_items SET views = views + 1 WHERE id = $id");
}
...
// ── LIKE HANDLING ──
if(isset($_GET['like'])){
    $conn->query("INSERT INTO badminton_likes (product_id) VALUES ($id)");
    header("Location: badminton_details.php?id=".$id);
    exit;
}
$likeCount = $conn->query("SELECT COUNT(*) as total FROM badminton_likes WHERE product_id=$id")->fetch_assoc()['total'];

// ── COMMENT HANDLING ──
if(isset($_POST['comment'])){
    $comment = $conn->real_escape_string($_POST['comment']);
    if($comment != ""){
        $conn->query("INSERT INTO badminton_comments (product_id, comment) VALUES ($id, '$comment')");
    }
}
$comments = $conn->query("SELECT * FROM badminton_comments WHERE product_id=$id ORDER BY id DESC");

// ── ADD TO CART / BUY NOW ──
if(isset($_POST['add_to_cart']) || isset($_POST['buy_now'])){
    if(!isset($_SESSION['user']['id'])){
        $redirect = urlencode("badminton_details.php?id=".$id);
    ...
    // For simplicity, store options as JSON
...
// ── TOP PICKS ──
$top_products = $conn->query("SELECT * FROM badminton_items WHERE is_top = 1 AND id != $id ORDER BY id DESC LIMIT 8");
$top_rows = [];
...
$redirect_url = urlencode("badminton_details.php?id=".$id);
...
<title><?php echo htmlspecialchars($data['title']); ?> — Badminton Gear | PlayZo</title>
...
<div class="product-wrap">
  ...
  <div class="info-section">
    ...
    <form method="POST" id="mainForm">
      <?php
      // helper to output attribute selection
      $selected_options = []; // will be filled via JS
      if($category == 'Badminton Jerseys' && isset($attributes['sizes'])){
          $sizes = $attributes['sizes'];
          if(is_array($sizes) && count($sizes) > 0){
              echo '<div class="attr-label"><i class="fa fa-ruler"></i> Select Size</div>';
              echo '<div class="attr-wrap" data-attr="size">';
              foreach($sizes as $sz){
                  echo '<div class="attr-btn" onclick="selectAttr(this, \'size\', \''.htmlspecialchars($sz).'\')">'.htmlspecialchars($sz).'</div>';
              }
              echo '</div>';
          }
          if(isset($attributes['colors']) && is_array($attributes['colors']) && count($attributes['colors']) > 0){
              echo '<div class="attr-label"><i class="fa fa-palette"></i> Select Color</div>';
              echo '<div class="attr-wrap" data-attr="color">';
              foreach($attributes['colors'] as $col){
                  echo '<div class="attr-btn" onclick="selectAttr(this, \'color\', \''.htmlspecialchars($col).'\')">'.htmlspecialchars($col).'</div>';
              }
              echo '</div>';
          }
      } elseif($category == 'Badminton Shoes' && isset($attributes['shoe_size'])){
          $shoe_size = $attributes['shoe_size'];
          echo '<div class="attr-label"><i class="fa fa-shoe-prints"></i> Shoe Size (EU)</div>';
          echo '<div class="attr-wrap" data-attr="shoe_size">';
          echo '<div class="attr-btn" onclick="selectAttr(this, \'shoe_size\', \''.htmlspecialchars($shoe_size).'\')">'.htmlspecialchars($shoe_size).'</div>';
          echo '</div>';
      } elseif($category == 'Nets'){
          $net_type = $attributes['net_type'] ?? '';
          $height   = $attributes['net_height'] ?? '';
          $width    = $attributes['net_width'] ?? '';
          if($net_type) echo '<div class="attr-label"><i class="fa fa-network-wired"></i> Net Type: <strong>'.htmlspecialchars($net_type).'</strong></div>';
          if($height) echo '<div class="attr-label"><i class="fa fa-arrows-up-down"></i> Height: '.htmlspecialchars($height).' m</div>';
          if($width) echo '<div class="attr-label"><i class="fa fa-arrows-left-right"></i> Width: '.htmlspecialchars($width).' m</div>';
          // For nets we still need a dummy attribute so that "Add to Cart" works
          echo '<input type="hidden" name="selected_options[net]" value="standard">';
      } else {
          // generic product: no attributes needed
          echo '<input type="hidden" name="selected_options[default]" value="1">';
      }
      ?>
      ...
      <div id="attr-alert" class="attr-alert"><i class="fa fa-circle-exclamation"></i> Please select all options first!</div>
...
<div class="top-picks-section">
  <div class="top-strip-hd">
    ...
    <a href="../publics/index.php?filter=Badminton">View All <i class="fa fa-arrow-right"></i></a>
  </div>
  <div class="tp-grid" id="tp-grid">
    ...
    <div class="tp-card">
      <div class="tp-cimg">
        ...
        <a href="badminton_details.php?id=<?php echo $tp['id']; ?>">
          <img src="<?php echo htmlspecialchars($tp_main); ?>" alt="<?php echo htmlspecialchars($tp['title']); ?>">
        </a>
        <a class="tp-qview" href="badminton_details.php?id=<?php echo $tp['id']; ?>"><i class="fa fa-eye"></i> Quick View</a>
      </div>
      <div class="tp-cbody">
        <div class="tp-ctitle"><?php echo htmlspecialchars($tp['title']); ?></div>
        <div class="tp-cclub"><?php echo htmlspecialchars($tp['store_name'] ?? 'PlayZo Nepal'); ?></div>
        <div class="tp-cmeta">
        ...
        <div class="tp-cfoot">
          ...
          <span class="tp-csport"><i class="fa fa-table-tennis-paddle-ball"></i> <?php echo $tp_sport; ?></span>
          <span class="tp-cstars"><?php for($i=1;$i<=5;$i++) echo $i<=$tp_avg?'★':'☆'; ?></span>
        </div>
        <a class="tp-cadd" href="badminton_details.php?id=<?php echo $tp['id']; ?>"><i class="fa fa-eye"></i> View Details</a>
      </div>
    </div>
    <?php endforeach; ?>
  </div>
...
// --- ATTRIBUTE SELECTION ---
let selectedAttrs = {};
function selectAttr(btn, attrName, value){
  // remove active from same group
  const parent = btn.parentNode;
  parent.querySelectorAll('.attr-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  selectedAttrs[attrName] = value;
  document.getElementById('selectedOptionsJson').value = JSON.stringify(selectedAttrs);
  // hide alert if any
  document.getElementById('attr-alert').style.display = 'none';
}
...
// --- FORM SUBMIT (only if all required attributes selected) ---
function submitForm(action){
  <?php if($category == 'Badminton Jerseys' || $category == 'Badminton Shoes'): ?>
    // For jerseys, need size (and maybe color); for shoes need shoe_size
    let required = <?php echo $category == 'Badminton Jerseys' ? '["size"]' : '["shoe_size"]'; ?>;
    for(let r of required){
      if(!selectedAttrs[r]){
        document.getElementById('attr-alert').style.display = 'block';
        return;
      }
    }
  <?php endif; ?>
