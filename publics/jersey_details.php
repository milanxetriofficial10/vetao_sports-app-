...
// ── UPDATED QUERY: JOIN with shops to get shop_name ──
$data = $conn->query("SELECT j.*, s.shop_name 
                      FROM jerseys j 
...
// ── LIKE ──
if(isset($_GET['like'])){
    $conn->query("INSERT INTO likes (jersey_id) VALUES ($id)");
    header("Location: jersey_details.php?id=".$id);
    exit;
}

$likeCount = $conn->query("SELECT COUNT(*) as total FROM likes WHERE jersey_id=$id")->fetch_assoc()['total'];

// ── COMMENT ──
if(isset($_POST['comment'])){
    $c = $conn->real_escape_string($_POST['comment']);
    if($c != ""){
        $conn->query("INSERT INTO comments (jersey_id, comment) VALUES ($id,'$c')");
    }
}

$comments = $conn->query("SELECT * FROM comments WHERE jersey_id=$id ORDER BY id DESC");

// ── ADD TO CART ──
if(isset($_POST['add_to_cart'])){
    // Must be logged in
    if(!isset($_SESSION['user']['id'])){
        $redirect = urlencode("jersey_details.php?id=".$id);
...
// ── TOP PICKS ──
$top_products = $conn->query("SELECT * FROM jerseys WHERE is_top = 1 AND id != $id ORDER BY id DESC LIMIT 8");
$top_rows = [];
...
$redirect_url = urlencode("jersey_details.php?id=".$id);

// Shop name to display (default "General Store" if not set)
...
.product-title{
  ...
  text-shadow:0 2px 8px rgba(95, 245, 8, 0.97);
}
...
.price-main{
  ...
  color: #16a34a;
  text-shadow:0 2px 8px rgba(203, 133, 206, 0.95);
}
.price-original{
  font-size:16px;
...
.comment-input:focus{
  border-color:rgba(109, 87, 87, 0.91);
  box-shadow:0 0 0 3px rgba(255,255,255,0.12);
  background:rgba(220, 49, 2, 0.97);
}
.post-btn{
  margin-top:10px;
  padding:11px 28px;
...
.top-strip-hd a{
  ...
  transition:.2s;white-space:nowrap;background:rgba(255,255,255,0.1);
}
...
<div class="login-modal-overlay" id="loginModal">
  <div class="login-modal">
    ...
    <div class="modal-sub">
      Order garna pehile account ma login garnus.<br>
      Login garisakepachi automatically yo jersey ma farkaucha.
    </div>

    <!-- product preview inside modal -->
    <div class="modal-product">
      <img src="<?php echo htmlspecialchars($data['image']); ?>" alt="<?php echo htmlspecialchars($data['title']); ?>">
      <div>
    ...
    <div class="modal-btns">
      <a href="login.php?redirect=<?php echo $redirect_url; ?>" class="modal-btn-login">
        <i class="fa fa-sign-in-alt"></i> Login Garnus
      </a>
      <a href="register.php?redirect=<?php echo $redirect_url; ?>" class="modal-btn-register">
        <i class="fa fa-user-plus"></i> Naya Account Banaunus
      </a>
      <button class="modal-skip" onclick="closeModal()">
        Pachi garchhu — Continue browsing
      </button>
    </div>
  </div>
</div>
...
<div class="product-wrap">
  ...
  <div class="img-section">
    <div class="main-img-outer">
      <div class="main-img-wrap" id="mainImgWrap">
        ...
        <?php endif; ?>
        <?php if(!empty($data['sell']) && $data['sell']=='Yes'): ?>
          <div class="img-sell">SELL</div>
        <?php endif; ?>
        <img id="mainImg" src="<?php echo $data['image']; ?>" alt="<?php echo htmlspecialchars($data['title']); ?>">
      </div>
      <div class="side-zoom" id="sideZoom">
        <img id="sideZoomImg" src="<?php echo $data['image']; ?>" alt="zoom">
    ...
    <div class="thumbs">
      <div class="thumb-wrap">
        <img src="<?php echo $data['image']; ?>" class="active" onclick="changeImg(this)">
        <div class="zoom-popup"><img src="<?php echo $data['image']; ?>"></div>
      </div>
      <?php foreach($extraImages as $img): $img = trim($img); ?>
      <div class="thumb-wrap">
        <img src="<?php echo $img; ?>" onclick="changeImg(this)">
        <div class="zoom-popup"><img src="<?php echo $img; ?>"></div>
      </div>
      <?php endforeach; ?>
    </div>
  </div>
  ...
  <div class="info-section">
    ...
    <div class="product-meta">
      <span class="meta-badge"><i class="fa fa-tshirt"></i> <?php echo htmlspecialchars($data['jersey_type']); ?></span>
    ...
    <form method="POST" id="mainForm">
      <div class="size-label"><i class="fa fa-ruler"></i> &nbsp;Select Size</div>
      <div class="size-wrap">
        <?php foreach(explode(",",$data['sizes']) as $s):
          $s = trim($s); ?>
          <div class="size-btn" onclick="selectSize(this,'<?php echo $s; ?>')"><?php echo $s; ?></div>
        <?php endforeach; ?>
      </div>
        ...
        <div class="share-wrap" id="shareWrap">
          ...
          <div class="share-dropdown" id="shareDropdown">
            ...
            <div class="share-icons">
              <a class="share-link facebook" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>" target="_blank">
                <i class="fab fa-facebook-f"></i> Facebook
              </a>
              <a class="share-link instagram" href="https://www.instagram.com/" target="_blank">
                <i class="fab fa-instagram"></i> Instagram
              </a>
              <a class="share-link twitter" href="https://twitter.com/intent/tweet?url=<?php echo urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>&text=<?php echo urlencode($data['title']); ?>" target="_blank">
      ...
      <!-- SIZE ALERT -->
      <div id="size-alert" style="display:none;">
        <i class="fa fa-circle-exclamation"></i> Please select a size first!
      </div>

      <!-- CTA BUTTONS -->
      <div class="btn-group" style="margin-top:16px;">
        <?php if($is_logged_in): ?>
          <!-- LOGGED IN: normal submit -->
        ...
        <?php else: ?>
          ...
          <button class="btn cart-btn" type="button" onclick="requireLogin()">
            <i class="fa fa-shopping-cart"></i> Add to Cart
          </button>
          <button class="btn buy-btn" type="button" onclick="requireLogin()">
            <i class="fa fa-bolt"></i> Buy Now
          </button>
        <?php endif; ?>
      </div>
      ...
      <div style="margin-top:10px;text-align:center;font-size:12.5px;color:rgba(255,255,255,0.55);">
        ...
        Order garna
        <a href="login.php?redirect=<?php echo $redirect_url; ?>" style="color:#fde68a;font-weight:700;">Login garnus</a>
        ya
        <a href="register.php?redirect=<?php echo $redirect_url; ?>" style="color:#fde68a;font-weight:700;">Register garnus</a>
      </div>
      <?php endif; ?>
...
<div class="comment-section">
  <div class="comment-card">
    ...
    <form method="POST">
      <textarea class="comment-input" name="comment" placeholder="Share your experience about this jersey..."></textarea>
...
<div class="top-picks-section">
  ...
  <div class="tp-grid" id="tp-grid">
    <?php foreach($top_rows as $tp):
      ...
      $tp_is_sell  = !empty($tp['sell'])     && $tp['sell']     == 'Yes';
      $tp_is_new   = !empty($tp['is_new'])   && $tp['is_new']   == 'Yes';
    ...
    <div class="tp-card">
      <div class="tp-cimg">
        ...
        <?php endif; ?>
        <a href="jersey_details.php?id=<?php echo $tp['id']; ?>">
          <img src="<?php echo htmlspecialchars($tp['image']); ?>" alt="<?php echo htmlspecialchars($tp['title']); ?>">
        </a>
        <a class="tp-qview" href="jersey_details.php?id=<?php echo $tp['id']; ?>">
          <i class="fa fa-eye"></i> Quick View
        </a>
      </div>
      <div class="tp-cbody">
        <div class="tp-ctitle"><?php echo htmlspecialchars($tp['title']); ?></div>
        <div class="tp-cclub"><?php echo htmlspecialchars($tp['description'] ?? 'SportGhar Collection'); ?></div>
        <div class="tp-cmeta">
          <span class="tp-ctype"><?php echo htmlspecialchars($tp['jersey_type']); ?></span>
        ...
        <div class="tp-cfoot">
          ...
          <span class="tp-csport">
            <?php
            $icons = ['Football'=>'fa-futbol','Basketball'=>'fa-basketball','Cricket'=>'fa-cricket-bat-ball',
                      'Badminton'=>'fa-table-tennis-paddle-ball','Volleyball'=>'fa-volleyball',
                      'Tennis'=>'fa-baseball-bat-ball','Boxing'=>'fa-hand-fist','Cycling'=>'fa-bicycle',
                      'Rugby'=>'fa-football','Esports'=>'fa-gamepad','Gym'=>'fa-dumbbell'];
            $ic = $icons[$tp_sport] ?? 'fa-shirt';
            echo '<i class="fa '.$ic.'"></i> '.$tp_sport;
            ?>
          </span>
          <span class="tp-cstars">
            <?php for($i=1;$i<=5;$i++) echo $i<=$tp_avg ? '★' : '☆'; ?>
          </span>
        </div>
        <a class="tp-cadd" href="jersey_details.php?id=<?php echo $tp['id']; ?>">
          <i class="fa fa-eye"></i> View Jersey
        </a>
      </div>
    </div>
    <?php endforeach; ?>
...
const REDIRECT_URL = "login.php?redirect=<?php echo $redirect_url; ?>";
